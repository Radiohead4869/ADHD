    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex justify-between items-end">
        <div>
          <h2 className="text-3xl font-bold tracking-tight flex items-center gap-3">
            <Anchor className="w-8 h-8 text-[var(--primary)]" />
            Sacred Seats
          </h2>
          <p className="text-[var(--text-dim)] mt-2">Define physical or digital anchors that demand 100% focus.</p>
        </div>
        <button 
      ...
      {isCreating && (
        <form onSubmit={handleCreate} className="bg-[var(--panel)] border border-[var(--border)] p-6 rounded-xl space-y-4">
          ...
          <div>
            <label className="block text-sm font-mono text-[var(--text-dim)] uppercase mb-1">Anchor Name</label>
            <input 
              type="text" 
          ...
          <div>
            <label className="block text-sm font-mono text-[var(--text-dim)] uppercase mb-1">Definition & Rules</label>
            <textarea 
              ...
              className="w-full bg-[var(--bg-dark)] border border-[var(--border)] rounded p-3 text-[var(--text-main)] focus:border-[var(--primary)] focus:outline-none transition-colors h-24 resize-none"
            />
          </div>

          <div>
            <label className="block text-sm font-mono text-[var(--text-dim)] uppercase mb-1 text-red-400">The Consequence (Crucial)</label>
            <textarea 
              ...
              className="w-full bg-[var(--bg-dark)] border border-red-900/50 rounded p-3 text-[var(--text-main)] focus:border-red-500 focus:outline-none transition-colors h-24 resize-none"
              required
            />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" className="bg-[var(--primary)] text-[var(--bg-main)] px-6 py-2 rounded font-bold uppercase text-sm tracking-wider">Establish</button>
          </div>
        </form>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {anchors.map(anchor => {
          ...
          return (
            <div key={anchor.id} className={`bg-[var(--panel)] border rounded-xl p-6 relative overflow-hidden transition-colors ${isSessionActive ? 'border-[var(--primary)]' : 'border-[var(--border-light)]'}`}>
              {isSessionActive && (
                <div className="absolute top-0 left-0 w-full h-1 bg-[var(--primary)] animate-pulse" />
              )}
              
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-xl font-bold">{anchor.name}</h3>
                  <div className="text-xs font-mono text-[var(--text-dark)] mt-1">Chain: #{anchor.chainLength}</div>
                </div>
                ...
                {isSessionActive ? (
                  ...
                  <button 
                    ...
                    className="bg-[var(--primary)]/10 text-[var(--primary)] hover:bg-[var(--primary)]/20 p-2 rounded flex items-center gap-2 text-sm font-bold uppercase disabled:opacity-50 disabled:cursor-not-allowed"
                  >
              ...
              <p className="text-[var(--text-dim)] text-sm mb-4">{anchor.description}</p>
              ...
              {anchor.allowedExceptions.length > 0 && (
                <div className="mt-4 pt-4 border-t border-[var(--border-light)]">
                  <span className="text-xs font-mono text-[var(--text-dark)] uppercase block mb-2">Permanent Exceptions</span>
      ...
      {activeExceptionModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--panel)] border border-[var(--border)] rounded-xl max-w-lg w-full p-6 shadow-2xl">
            <div className="flex items-center gap-3 text-red-500 mb-4">
              <ShieldAlert className="w-8 h-8" />
              <h3 className="text-2xl font-bold">Jurisprudence Selector</h3>
            </div>
            
            <p className="text-[var(--text-muted)] mb-6">You are about to leave the sacred seat. Choose your consequence:</p>

            <div className="space-y-4">
              <div className="border border-[var(--border)] rounded-lg p-4 hover:border-red-500 transition-colors cursor-pointer" onClick={() => submitException(activeExceptionModal, 'break')}>
                <div className="font-bold text-red-400 mb-1">[A] Break the Chain</div>
                <p className="text-sm text-[var(--text-dim)]">Terminate session immediately. Chain resets to #0. Tomorrow starts from #1.</p>
              </div>

              <div className="border border-[var(--border)] rounded-lg p-4 hover:border-yellow-500 transition-colors">
                <div className="font-bold text-yellow-400 mb-1">[B] Establish Precedent (Permanent Exception)</div>
                <p className="text-sm text-[var(--text-dim)] mb-3">Allow this behavior, but it will be PERMANENTLY allowed for the lifetime of this anchor.</p>
                <input 
                  type="text"
                  value={exceptionReason}
                  onChange={e => setExceptionReason(e.target.value)}
                  placeholder="Define the exact exception (e.g., 'Bathroom break under 2 mins')"
                  className="w-full bg-[var(--bg-dark)] border border-[var(--border)] rounded p-2 text-sm text-[var(--text-main)] focus:border-yellow-500 focus:outline-none"
                />
                <button 
            ...
            <button onClick={() => setActiveExceptionModal(null)} className="mt-6 w-full text-[var(--text-dark)] hover:text-white text-sm">Cancel (Return to Seat)</button>
          </div>
        </div>
      )}
    </div>
